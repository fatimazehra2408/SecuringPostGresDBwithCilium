...
const pgresHdrLen = 9
const pgresMaxLen = 268435456 // 256 MB, per spec
...
func PostgresRuleParser(rule *cilium.PortNetworkPolicyRule) []L7NetworkPolicyRule {
	...
	for _, l7Rule := range allowRules {
		...
		for k, v := range l7Rule.Rule {
			switch k {
			case "query_action":
				pg.queryActionExact = v
			case "query_table":
				if v != "" {
					pg.tableRegexCompiled = regexp.MustCompile(v)
				}
			default:
				ParseError(fmt.Sprintf("Unsupported key: %s", k), rule)
			}
		}
		if len(pg.queryActionExact) > 0 {
			// ensure this is a valid query action
			res := queryActionMap[pg.queryActionExact]
			if res == invalidAction {
				ParseError(fmt.Sprintf("Unable to parse L7 postgres rule with invalid query_action: '%s'", pg.queryActionExact), rule)
			} else if res == actionNoTable && pg.tableRegexCompiled != nil {
				ParseError(fmt.Sprintf("query_action '%s' is not compatible with a query_table match", pg.queryActionExact), rule)
			}

		}

		log.Debugf("Parsed PostgresRule pair: %v", pg)
		rules = append(rules, &pg)
	}
	return rules
}
...
func (pf *postgresParserFactory) Create(connection *Connection) interface{} {
	log.Debugf("postgresParserFactory: Create: %v", connection)
...
func (p *postgresParser) OnData(reply, endStream bool, dataArray [][]byte) (OpType, int) {
	...
	if len(data) < pgresHdrLen {
		// Partial header received, ask for more
		needs := pgresHdrLen - len(data)
	...
	if reply {
		...
		postgresParseReply(p, data[0:(pgresHdrLen+requestLen)])

		log.Debugf("reply, passing %d bytes", (pgresHdrLen + requestLen))
		return PASS, (pgresHdrLen + int(requestLen))
	}

	err, paths := postgresParseRequest(p, data[0:(pgresHdrLen+requestLen)])
	if err != 0 {
	...
	if !matches {
		...
		return DROP, int(pgresHdrLen + requestLen)
	}

	return PASS, int(pgresHdrLen + requestLen)
}
