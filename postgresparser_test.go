...
func (PostgresSuite) checkAccessLogs(c *C, expPasses, expDrops int) {
...
// util function used for Cassandra tests, as we have postgres requests
// as hex strings
...
func (s *Suite) TestPostgresOnDataNoHeader(c *C) {
	conn := s.ins.CheckNewConnectionOK(c, "postgres", true, 1, 2, "1.1.1.1:34567", "2.2.2.2:80", "no-policy")
	data := hexData(c, "0400")
...
func (s *PostgresSuite) TestPostgresBatchRequestPreparedStatement(c *C) {
	...
	pgresParser, ok := (conn.Parser).(*PostgresParser)
	if !ok {
		panic("failed to cast conn.Parser to *PostgresParser\n")
	}
	preparedQueryID1 := "aaaa"
	pgresParser.preparedQueryPathByPreparedID[preparedQueryID1] = "/batch/select/db3.t1"
	preparedQueryID2 := "bbbb"
	pgresParser.preparedQueryPathByPreparedID[preparedQueryID2] = "/batch/select/db3.t2"
...
func (s *PostgresSuite) TestPostgresBatchRequestPreparedStatementDenied(c *C) {
	...
	pgresParser.preparedQueryPathByPreparedID[preparedQueryID1] = "/batch/select/db3.t1"
	preparedQueryID2 := "bbbb"
	pgresParser.preparedQueryPathByPreparedID[preparedQueryID2] = "/batch/select/db4.t2"
...
func (s *PostgresSuite) TestPostgresExecutePreparedStatement(c *C) {
	...
	preparedQueryID1 := "aaaa"
	pgresParser.preparedQueryPathByPreparedID[preparedQueryID1] = "/query/select/db3.t1"
...
func (s *PostgresSuite) TestPostgresPreparedResultReply(c *C) {
	...
	pgresParser.preparedQueryPathByStreamID[uint16(4)] = "/query/select/db3.t1"
